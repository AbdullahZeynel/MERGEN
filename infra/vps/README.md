...
`mergen` kullanıcısı tarafından okunabilmeli, Caddy'nin `current/shared`
dizinleri altında olmamalıdır. `/etc/mergen/services.env` içindeki
`MERGEN_DEMO_ROOT` değerini `/srv/mergen/demo-v3` olarak ayarlayın. Örnek dosyada
gerçek makine bilgisi yoktur. Kod güncellemesi eski v2 paketi okumaya devam eder;
v3 pakete geçiş yalnız bu ortam değişkeni değiştirilip MCP yeniden başlatıldığında
gerçekleşir.
