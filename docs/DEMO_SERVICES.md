...
```bash
MERGEN_DEMO_ROOT="$PWD/.local/demo-v2" backend/.venv/bin/python mcp/server.py
```
```bash
...
MCP bilinmeyen vaka, eksen, sınır dışı indeks ve dizin dışına çıkan yolları reddeder. Varlık başına 8 MiB sınırı, gateway'de SHA-256 denetimi ve 15 saniye zaman aşımı vardır. Aynı anda dört MCP çağrısı işlenir; bu **10 kullanıcı oturum sınırı değildir**. Oturum/auth ve gerçek kapasite ölçümü sonraki sprinttedir. Bu uçlar yalnızca yayımlanabilir demo verisi içindir; gerçek hasta verisi yükleme yoktur. API hatası demo/canlı geçişini tetiklemez. Canlı modeller, genomik sonuç eşleştirme ve chatbot henüz bağlı değildir.
